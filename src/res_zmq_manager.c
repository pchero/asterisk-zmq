...
static json_t*  g_json_res;  //!< action cmd response
...
static int zmq_cmd_helper(int category, const char *event, char *content)
{
    ...
    json_object_set(g_json_res, key, json_string(value));
    return 0;
}
 ...
 */
static json_t* recv_parse(char* msg)
{
    json_t* j_out;
    json_error_t error;

    j_out = json_loads(msg, 0, &error);
    if(j_out == NULL)
    {
...
static void zmq_cmd_thread(void)
{
    ...
    while (1)
    {
        ...
        zmq_data->zmq_sock = g_app.addr_cmd;
...
static void zmq_cmd_handler(void *data)
{
    ...
    DEBUG("ast_worker!! data[%p]\n", data);

    zmq_data = (zmq_data_t*)data;

    dumps = json_dumps(zmq_data->j_recv, JSON_INDENT(1));
    ...
    // Get action
    j_tmp = json_object_get(zmq_data->j_recv, "action");
    if(j_tmp == NULL)
    {
        ERROR("%s\n", "Could not get the action.");
    }
    sprintf(str_cmd, "action:%s\n", json_string_value(j_tmp));

    json_object_foreach(zmq_data->j_recv, key, value)
    {
...
static int zmq_evt_helper(int category, const char *event, char *content)
{
    ...
    for(i = 0; i < cnt; i++)
    {
        ...
        trim(key);
        trim(value);
        j_tmp = json_string(value);
        json_object_set(j_out, key, j_tmp);
        json_decref(j_tmp);

        DEBUG("parse[%s], key[%s], value[%s]\n", parse[i], key, value);
    }

    buf_send = json_dumps(j_out, JSON_INDENT(0));
